...
First verify the offline fixture:

```sh
node ENGINE/src/cli.mjs analyze --fixture ENGINE/examples/archive.json --out WORK/output/demo
```

Then process validated rows sequentially, using a unique output folder per analysis key (normalized domain + analysis date):

```sh
node ENGINE/src/cli.mjs analyze --domain example.com --as-of 2025-08-14 --out WORK/output/row-001
```

Replace ENGINE and WORK with actual paths; invoke the process using an argument array or properly quote paths and untrusted input. Never interpolate a CSV cell as shell code. Store outputs in a working folder, outside the installed skill. Preserve row/deal identity when the same company has multiple close dates. Reuse an analysis only for the exact same domain and cutoff.
